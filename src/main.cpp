...
uint32_t outputNeopix(float freqNeopix, uint32_t isrTime, float phaseDiff) {
  const uint32_t maxDutyNeoPix = 255; // brightness of LED : 0 to 255
...
void taskOnAppCPU(void *pvParameters) {
  while (1) {
    ...
    switch (pwm.stt_waveID) {
      case SIN_WAVE: // 3rd 5*LED =  9, 10, 11, 12, 13
        freq = 0.5f;
        strip.setPixelColor( 9, 0, 0, outputNeopix(freq, isrTime, 0.0f) );
        strip.setPixelColor(10, 0, 0, outputNeopix(freq, isrTime, 0.3f) );
        strip.setPixelColor(11, 0, 0, outputNeopix(freq, isrTime, 0.6f) );
        strip.setPixelColor(12, 0, 0, outputNeopix(freq, isrTime, 0.9f) );
        strip.setPixelColor(13, 0, 0, outputNeopix(freq, isrTime, 1.2f) );
        break;
      case SAW_TOOTH: // 2nd 5*LED =  4,  5,  6,  7,  8
        freq = 0.5f;
        strip.setPixelColor( 4, 0, 0, outputNeopix(freq, isrTime, 0.0f) );
        strip.setPixelColor( 5, 0, 0, outputNeopix(freq, isrTime, 0.3f) );
        strip.setPixelColor( 6, 0, 0, outputNeopix(freq, isrTime, 0.6f) );
        strip.setPixelColor( 7, 0, 0, outputNeopix(freq, isrTime, 0.9f) );
        strip.setPixelColor( 8, 0, 0, outputNeopix(freq, isrTime, 1.2f) );
        break;
      case PWM12P5: // 1st 4*LED =  0,  1,  2,  3
        freq = 0.5f;
        strip.setPixelColor( 0, 0, 0, outputNeopix(freq, isrTime, 0.0f) );
        strip.setPixelColor( 1, 0, 0, outputNeopix(freq, isrTime, 0.3f) );
        strip.setPixelColor( 2, 0, 0, outputNeopix(freq, isrTime, 0.6f) );
        strip.setPixelColor( 3, 0, 0, outputNeopix(freq, isrTime, 0.9f) );
        break;
      case PWM25: // 7th 6*LED = 30, 31, 32, 33, 34, 35
        freq = 0.5f;
        strip.setPixelColor(30, 0, 0, outputNeopix(freq, isrTime, 0.0f) );
        strip.setPixelColor(31, 0, 0, outputNeopix(freq, isrTime, 0.3f) );
        strip.setPixelColor(32, 0, 0, outputNeopix(freq, isrTime, 0.6f) );
        strip.setPixelColor(33, 0, 0, outputNeopix(freq, isrTime, 0.9f) );
        strip.setPixelColor(34, 0, 0, outputNeopix(freq, isrTime, 1.2f) );
        strip.setPixelColor(35, 0, 0, outputNeopix(freq, isrTime, 1.5f) );
        break;
      case SQUARE: // 6th 6*LED = 24, 25, 26, 27, 28, 29
        freq = 0.5f;
        strip.setPixelColor(24, 0, 0, outputNeopix(freq, isrTime, 0.0f) );
        strip.setPixelColor(25, 0, 0, outputNeopix(freq, isrTime, 0.3f) );
        strip.setPixelColor(26, 0, 0, outputNeopix(freq, isrTime, 0.6f) );
        strip.setPixelColor(27, 0, 0, outputNeopix(freq, isrTime, 0.9f) );
        strip.setPixelColor(28, 0, 0, outputNeopix(freq, isrTime, 1.2f) );
        strip.setPixelColor(29, 0, 0, outputNeopix(freq, isrTime, 1.5f) );
        break;
      case PSEUDO_TRIANGLE: // 5th 5*LED = 19, 20, 21, 22, 23
        freq = 0.5f;
        strip.setPixelColor(19, 0, 0, outputNeopix(freq, isrTime, 0.0f) );
        strip.setPixelColor(20, 0, 0, outputNeopix(freq, isrTime, 0.3f) );
        strip.setPixelColor(21, 0, 0, outputNeopix(freq, isrTime, 0.6f) );
        strip.setPixelColor(22, 0, 0, outputNeopix(freq, isrTime, 0.9f) );
        strip.setPixelColor(23, 0, 0, outputNeopix(freq, isrTime, 1.2f) );
        break;
      case TRIANGLE: // 4th 5*LED = 14, 15, 16, 17, 18
        freq = 0.5f;
        strip.setPixelColor(14, 0, 0, outputNeopix(freq, isrTime, 0.0f) );
        strip.setPixelColor(15, 0, 0, outputNeopix(freq, isrTime, 0.3f) );
        strip.setPixelColor(16, 0, 0, outputNeopix(freq, isrTime, 0.6f) );
        strip.setPixelColor(17, 0, 0, outputNeopix(freq, isrTime, 0.9f) );
        strip.setPixelColor(18, 0, 0, outputNeopix(freq, isrTime, 1.2f) );
        break;
    }
