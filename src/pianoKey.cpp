...
const float freqList[MULTIPLEX_NUM][MULTIPLEX_CH_NUM] = {
    {-1.000, 38.891, 41.203, 43.654, 46.249, 48.999, 51.913, 55.000, 36.708, 61.735, 65.406, 69.296, 34.648, 32.703},
    {73.416, 77.782, 82.407, 87.307, 92.499, 97.999, 103.83, 110.00, 116.54, 123.47, 130.81, 138.59, -1.000, -1.000},
    {146.83, 155.56, 164.81, 174.61, 185.00, 196.00, 207.65, 220.00, 233.08, 246.94, 261.63, 277.18, -1.000, -1.000},
    {293.67, 311.13, 329.63, 349.23, 369.99, 392.00, 415.31, 440.00, 466.16, 493.88, 523.25, 554.37, -1.000, -1.000},
    {587.33, 622.25, 659.26, 698.46, 739.99, 783.99, 830.61, 880.00, 932.33, 987.77, 1046.5, 1108.7, -1.000, -1.000},
    {1174.7, 1244.5, 1318.5, 1396.9, 1480.0, 1568.0, 1661.2, 1760.0, 1864.7, 1975.5, 2093.0, 2217.5, -1.000, -1.000},
    {2349.3, 2489.0, 2637.0, 2793.8, 2960.0, 3136.0, 3322.4, 3520.0, 3729.3, 3951.1, 4186.0, 4434.9, 4698.6, 5274.0},
};
...
void PianoKey::process(uint32_t multiplx, uint32_t multiplxCH) {
  polling(multiplx, multiplxCH);
  stateLower(multiplx, multiplxCH);  // Uppwer接点んが機能せずLower接点のみが機能
  // state(multiplx, multiplxCH);//Upper接点、Lower接点の両方ともに機能
}

void PianoKey::polling(uint32_t multiplx, uint32_t multiplxCH) {
  ...
  */
  ppianokey->upper = digitalRead(upperDIN[multiplx]);
  ppianokey->lower = digitalRead(lowerDIN[multiplx]);
}
